...
import { AGENCY_ROLE_OPTIONS, getAgencyTeamMemberName, type AgencyTeamMember } from '../config/teamMemberConfig'
...
export function AssignTeamMemberRoleDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(t('users.updateRoleFailed', { defaultValue: 'Failed to update member role' }))
    }
  }

  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth='xs'>
      <DialogTitle>{t('users.assignRole', { defaultValue: 'Assign role' })}</DialogTitle>
      <DialogContent>
        <Stack spacing={2}>
          {member ? (
