...
export function RoomProfileSkeleton({ onBack, onEdit, onDelete }: RoomProfileSkeletonProps) {
  return (
    <Stack gap={2.5} sx={{ width: 1, maxWidth: 1120, mx: "auto" }}>
      <RoomProfileHeader title="" status="available" onBack={onBack} loading />
      <Grid container spacing={2.5}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <RoomInfoCard
            room={{ type: "single", floorNumber: 0, capacity: 0, bedType: "single", pricePerNight: undefined, starRating: 0 }}
            onEdit={onEdit}
            onDelete={onDelete}
            loading
          />
