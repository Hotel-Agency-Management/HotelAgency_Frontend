...
import { DEFAULT_BRANDING_COLORS, type BrandingColors } from '@/core/theme/palette/branding'
    ...
    <Paper elevation={0} variant='card'>
      <Stack spacing={0.5}>
        <Typography variant='subtitle1' fontWeight={500}>
          {t('agencySettings.theme.colorsTitle', { defaultValue: 'Theme colors' })}
        </Typography>
        <Typography variant='body2'>
          {t('agencySettings.theme.colorsHint', { defaultValue: 'Click any swatch to open the color picker.' })}
        </Typography>
      </Stack>
      <Divider />
        ...
        {colorFields.map(({ name, label, hint }) => {
          ...
          return (
            <Grid key={fullName} size={{ xs: 12, md: 4 }}>
              <ColorField name={fullName} label={label} helperText={hint} fallbackColor={fallbackColors[colorKey]} />
            </Grid>
          )
        })}
      </Grid>
