...
export function useCreateHotelTerms() {
  ...
  return useMutation<TermsResponse, unknown, CreateTermsVariables>({
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(t('terms.toast.created', { defaultValue: 'Terms & Conditions created successfully' }))
    },
    onError: error => {
      toast.error(
        getErrorMessage(error, t('terms.toast.createFailed', { defaultValue: 'Failed to create Terms & Conditions' }))
      )
    }
  })
}
...
export function useUpdateHotelTerms() {
  ...
  return useMutation<TermsResponse, unknown, UpdateTermsVariables>({
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(t('terms.toast.updated', { defaultValue: 'Terms & Conditions updated successfully' }))
    },
    onError: error => {
      toast.error(
        getErrorMessage(error, t('terms.toast.updateFailed', { defaultValue: 'Failed to update Terms & Conditions' }))
      )
    }
  })
}
...
export function useAdminCreateHotelTerms() {
  ...
  return useMutation<TermsResponse, unknown, CreateAdminTermsVariables>({
    mutationFn: ({ agencyId, hotelId, data }) => adminCreateHotelTerms(agencyId, hotelId, data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
...
export function useAdminUpdateHotelTerms() {
  ...
  return useMutation<TermsResponse, unknown, UpdateAdminTermsVariables>({
    mutationFn: ({ agencyId, hotelId, id, data }) => adminUpdateHotelTerms(agencyId, hotelId, id, data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: hotelTermsQueryKeys.adminList(variables.agencyId, variables.hotelId)
      })
      queryClient.invalidateQueries({
        queryKey: hotelTermsQueryKeys.adminDetail(variables.agencyId, variables.hotelId, variables.id)
      })
      toast.success(t('terms.toast.updated', { defaultValue: 'Terms & Conditions updated successfully' }))
    },
    onError: error => {
