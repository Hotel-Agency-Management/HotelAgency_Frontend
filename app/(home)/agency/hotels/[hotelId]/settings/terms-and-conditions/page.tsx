...
export default function HotelTermsConditionsPage() {
  ...
  return (
    <HotelSettingsShell hotelId={hotelId}>
      {isLoading && !hotel ? (
        <Alert severity="info">{t('hotelSettings.profile.loadingDetails', 'Loading hotel details...')}</Alert>
      ) : hotel ? (
        ...
        <Alert severity="error">{t('hotelSettings.profile.notFound', 'Hotel not found.')}</Alert>
      )}
    </HotelSettingsShell>
  );
}
