...
export default function ReportDamageDialog({ open, onClose, onSubmit, prefill }: ReportDamageDialogProps) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      <DialogTitle>{t('damageReports.reportDamage', { defaultValue: 'Report Room Damage' })}</DialogTitle>
      <DialogContent>
        <Stack gap={2}>
          <Typography variant='body2'>
            {t('damageReports.room', { defaultValue: 'Room' })}: <strong>{prefill.roomNumber}</strong>
          </Typography>
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                {...field}
                label={t('damageReports.description', { defaultValue: 'Damage Description' })}
                multiline
                rows={3}
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                {...field}
                select
                label={t('damageReports.severity', { defaultValue: 'Severity' })}
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                {...field}
                label={t('damageReports.estimatedCost', { defaultValue: 'Estimated Cost' })}
                type='number'
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit(onValid)} variant='contained' color='error'>
          {t('damageReports.submitReport', { defaultValue: 'Submit Report' })}
        </Button>
      </DialogActions>
    </Dialog>
  )
