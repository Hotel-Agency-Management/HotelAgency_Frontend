...
export function getDamageStatusLabels(t: TFunction): Record<DamageReportStatus, string> {
  return {
    [DAMAGE_REPORT_STATUS.REPORTED]: t('damageReports.status.Reported', { defaultValue: 'Reported' }),
    [DAMAGE_REPORT_STATUS.PENDING_REVIEW]: t('damageReports.status.PendingReview', { defaultValue: 'Pending Review' }),
    [DAMAGE_REPORT_STATUS.ESCALATED]: t('damageReports.status.Escalated', { defaultValue: 'Escalated' }),
    [DAMAGE_REPORT_STATUS.INSURED]: t('damageReports.status.Insured', { defaultValue: 'Covered by Insurance' }),
    [DAMAGE_REPORT_STATUS.INVOICED]: t('damageReports.status.Invoiced', { defaultValue: 'Invoiced' })
  }
}

export function getDamageSeverityLabels(t: TFunction): Record<DamageSeverity, string> {
  return {
    [DAMAGE_SEVERITY.LOW]: t('damageReports.severityLabels.LOW', { defaultValue: 'Low' }),
    [DAMAGE_SEVERITY.MEDIUM]: t('damageReports.severityLabels.MEDIUM', { defaultValue: 'Medium' }),
    [DAMAGE_SEVERITY.HIGH]: t('damageReports.severityLabels.HIGH', { defaultValue: 'High' })
  }
}
