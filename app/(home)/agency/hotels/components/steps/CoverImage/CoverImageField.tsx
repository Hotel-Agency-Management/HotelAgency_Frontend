...
export function CoverImageField() {
  ...
  const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setError(t('agencyHotels.coverImage.invalidType', { defaultValue: 'Please choose an image file.' }))
      e.target.value = ''
      return
    }

    if (file.size > MAX_COVER_SIZE) {
      setError(t('agencyHotels.coverImage.tooLarge', { defaultValue: 'Image must be 5 MB or less.' }))
      e.target.value = ''
      return
    }
    ...
    try {
      ...
      setError(t('agencyHotels.coverImage.uploadFailed', { defaultValue: 'Upload failed.' }))
    } finally {
      e.target.value = ''
    }
  }
  ...
  return (
    <Stack spacing={1}>
      <Typography variant='overline'>{t('agencyHotels.coverImage.label', { defaultValue: 'Cover image' })}</Typography>

      <CoverUploadBox hasImage={!!coverImage} onClick={() => !coverImage && fileInputRef.current?.click()}>
        {coverImage ? (
          <CoverImagePreview
            src={coverImage}
      ...
      <input ref={fileInputRef} hidden type='file' accept='image/png,image/jpeg,image/webp' onChange={handleUpload} />
