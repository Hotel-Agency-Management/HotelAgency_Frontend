...
export function ManagerStep({ isFirst, isLast, isSubmitting, mode, onBack, onNext }: ManagerStepProps) {
  ...
  const { control, watch, trigger } = useFormContext<HotelFormValues>()
  ...
  return (
    <StepLayout
      title={t('agencyHotels.steps.manager.title', { defaultValue: 'Manager assignment' })}
      subtitle={t('agencyHotels.steps.manager.subtitle', {
        defaultValue:
          'Choose the hotel manager from the agency team. Team members are managed separately in User Management.'
      })}
      isFirst={isFirst}
      isLast={isLast}
      ...
      <Stack spacing={2.5}>
        {eligibleManagers.length === 0 && (
          <Alert
            ...
            variant='outlined'
            action={
              <Button component={Link} href='/users' size='small' color='inherit'>
                {t('agencyHotels.steps.manager.openUsers', { defaultValue: 'Open users' })}
              </Button>
            }
          >
            {t('agencyHotels.steps.manager.noManagersAlert', {
              defaultValue: 'Add at least one Admin or Manager in User Management before creating the hotel.'
            })}
          </Alert>
        )}
        ...
        <Grid container spacing={2.5}>
          <Grid size={{ xs: 12 }}>
            <Controller
              ...
              control={control}
              rules={{
                required: t('agencyHotels.form.managerRequired', { defaultValue: 'Manager selection is required' })
              }}
              render={({ field, fieldState }) => (
                <TextField
                  ...
                  fullWidth
                  size='small'
                  label={t('agencyHotels.form.manager', { defaultValue: 'Hotel manager' })}
                  disabled={managerOptions.length === 0}
                  error={!!fieldState.error}
                  helperText={
                    fieldState.error?.message ??
                    t('agencyHotels.form.managerHelperText', {
                      defaultValue: 'Only team members with property manager roles can be assigned.'
                    })
                  }
                >
                  <MenuItem value='' disabled>
                    {t('agencyHotels.form.selectManager', { defaultValue: 'Select a manager' })}
                  </MenuItem>
            ...
            <Stack spacing={1}>
              <Stack direction='row' justifyContent='space-between' alignItems='center'>
                <Typography variant='subtitle2' fontWeight={600}>
                  {t('agencyHotels.steps.manager.selectedManager', { defaultValue: 'Selected manager' })}
                </Typography>
              ...
              <Typography variant='body2'>{selectedManager.email}</Typography>
