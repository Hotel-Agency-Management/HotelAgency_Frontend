...
export function CustomerRoomBookingCard({
  ...
  const estimatedTotalLabel = formatCardCurrency(reservationSummary.estimatedTotal, language, reservation.currency)
        ...
        <Stack gap={1.5} justifyContent='space-between' sx={{ width: 1, flexGrow: 1 }}>
            ...
            <Stack direction='row' justifyContent='space-between' alignItems='flex-start' gap={2}>
              ...
              <Stack alignItems='flex-end' gap={0.25} flexShrink={0}>
                ...
                <Typography variant='caption' fontWeight={700}>
                  {t('hotelPortal.card.perNight', { defaultValue: 'per night' })}
                </Typography>
              </Stack>
            </Stack>
          ...
          <Stack gap={1.5}>
            <Stack direction='row' justifyContent='space-between' alignItems='flex-end' gap={2}>
              ...
              <Typography variant='body2' fontWeight={700}>
                {t('hotelPortal.booking.estimatedTotal', { defaultValue: 'estimated total' })}
              </Typography>
            </Stack>

            {!isBookable ? (
              <Alert severity='warning'>
                {t('hotelPortal.booking.cannotBookMaintenance', {
                  defaultValue: 'This room cannot be booked right now because it is under maintenance or blocked.'
                })}
              </Alert>
            ) : null}
            ...
            {room.pricePerNight == null ? (
              <Alert severity='info'>
                {t('hotelPortal.booking.noNightlyRate', {
                  defaultValue: 'This room needs a nightly rate before it can be reserved.'
                })}
              </Alert>
            ) : null}
            ...
            {draftAvailabilityConflict != null ? (
              <Alert severity='error'>
                {t('hotelPortal.booking.alreadyBooked', {
                  checkIn: draftAvailabilityConflict.checkIn,
                  checkOut: draftAvailabilityConflict.checkOut,
                  defaultValue: 'This room is already booked between {{checkIn}} and {{checkOut}}.'
                })}
              </Alert>
            ) : null}
            ...
            {currentReservation ? (
              <Alert severity='success'>
                {t('hotelPortal.booking.existingReservation', {
                  defaultValue:
                    'You already have an active reservation for this room. Choose different dates to book another stay, or use the reservation section below to manage the existing booking.'
                })}
              </Alert>
            ) : null}

            <Button fullWidth variant='contained' disabled={isReserveDisabled} onClick={openConfirm}>
              {t('hotelPortal.booking.reserveRoom', { defaultValue: 'Reserve this room' })}
            </Button>
          </Stack>
        </Stack>
