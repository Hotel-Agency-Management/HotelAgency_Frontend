...
export default function MomentumWall() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const copy = sectionRef.current?.querySelector('.momentum-copy')
      const cards = gsap.utils.toArray<HTMLElement>('.momentum-card', sectionRef.current)
      ...
      cards.forEach((card, index) => {
        gsap.from(card, {
          x: cardDirections[index] * 46,
          y: 32,
          autoAlpha: 0,
          rotate: cardDirections[index] * 2.4,
          duration: 0.82,
          ease: 'power3.out',
  ...
  return (
    <section id='momentum-wall' ref={sectionRef} style={{ maxWidth: '1200px', margin: '0 auto', padding: '120px 24px' }}>
      <div className='momentum-layout' style={{ display: 'grid', gridTemplateColumns: '0.9fr 1.1fr', gap: '32px', alignItems: 'start' }}>
        ...
        <div className='momentum-grid' style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '18px' }}>
          {section.cards.map((card, index) => (
            <article
              key={card.title}
              className={`momentum-card momentum-card--${index + 1}`}
              style={{
                position: 'relative',
